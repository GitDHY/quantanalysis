...
from tests.backtest.test_baseline import _dummy_coverage


def _run(engine, prices, strategy_func):
    with patch.object(engine.data_fetcher, "fetch_prices", return_value=prices), \
         patch.object(engine, "validate_data_coverage", return_value=_dummy_coverage(prices)):
...
def test_zero_cost_preserves_equity(synthetic_prices, cost_free_config):
    ...
    expected = 145_271.4470  # legacy engine output, see Task 3 investigation
...
def test_fixed_cost_subtracts_from_equity(synthetic_prices, fixed_cost_config):
    """
    ...
    """
    engine_zero = BacktestEngine(BacktestConfig(
        start_date=fixed_cost_config.start_date,
        end_date=fixed_cost_config.end_date,
        initial_capital=fixed_cost_config.initial_capital,
        rebalance_freq=fixed_cost_config.rebalance_freq,
